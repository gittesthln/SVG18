...
\Elm{tspan}を用います。
\ShowFig{0.54}{ht}{svg-text-with-tspan}
...
\ShowFig{0.6}{ht}{svg-text-along-path}
...
%       の文字列を定義しています。
 \item \Elm{text}内に\Elm{textPath}を記述する
       ことで文字列をパスに沿って配置しています(\Line{Path1S}から
       \Line{Path1E})。この要素内で、上で定義した道程を属性\Attrib{xlink:href}で引用し、
